<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ApplyT.aspx.cs" Inherits="eReimbursement.ApplyT" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        var PostData = function () {
            ...
            var temps = parentStore.query('Type', 'T').items;
            ...
            for (var i = 0; i < Store1.getAllRange().length; i++) {
                ...
                newrecord.data.Type = 'T';
                newrecord.data.AccountCode = "62011900";
                ...
                if (getCookie('lang') != undefined && getCookie('lang').toLowerCase() == 'zh-cn') {
                    newrecord.data.SubType = '交通费';
                }
                else {
                    newrecord.data.SubType = 'Transportation';
                }
...
//            LabelPersonBG.setText(''); LabelStationBG.setText(''); LabelDepartmentBG.setText(''); LabelCOA.setText(''); LabelBudget.setText('');
            if (a == 'Update') {
                if (hdStore1id.getValue() != undefined && hdStore1id.getValue() != '') {
                    ...
                    Store1.getById(id).set('Fcity', txtFcity.getValue());
                    Store1.getById(id).set('Tcity', txtTcity.getValue());
...
//                    Store1.getById(id).set('Budget', cbxBudget.getValue()); //是否强制预算外
                ...
                newrecord.data.Fcity = txtFcity.getValue();
                newrecord.data.Tcity = txtTcity.getValue();
...
//                newrecord.data.Budget = cbxBudget.getValue();
        ...
        var getDetail = function (row) {
            ...
            txtFcity.setValue(row.data.Fcity);
            txtTcity.setValue(row.data.Tcity);
...
////            hdBudgetCurrent.setValue(row.data.BudgetCurrent.toString());
//            LabelBudget.setText(row.data.BudgetCurrent);
//            LabelPersonBG.setText(''); LabelStationBG.setText(''); LabelDepartmentBG.setText(''); LabelCOA.setText(''); LabelBudget.setText('');
            //如果是草稿,则计算除当前明细外的金额合计
//            var sum = 0;
//            for (var i = 0; i < Store1.getAllRange().length; i++) {
//                var record = Store1.getAllRange()[i].data;
//                if (!record.Budget) {
...
<body>
    ...
    <ext:Hidden ID="hdStationBudgetUsed" runat="server" />
    <ext:Hidden ID="hdStationBudget" runat="server" />
    ...
    <ext:Panel ID="Panel3" runat="server" Height="510" Width="880" Border="false" Padding="10"
        ...
        <Items>
            <ext:Panel ID="Panel5" runat="server" Height="190" X="10" Y="10" Border="false" Layout="FitLayout">
                <Items>
                            ...
                            <ext:LayoutColumn ColumnWidth="1">
                                <ext:Panel ID="Panel43" runat="server" Padding="10" Title="<%$ Resources:LocalText,ExpenseDetail%>"
                                    ...
                                    <Items>
                                        ...
                                        <ext:TextField ID="txtFcity" runat="server" FieldLabel="<%$ Resources:LocalText,From%>"
                                            LabelWidth="70" Width="300" X="10" Y="40" MaxLength="50">
                                        </ext:TextField>
                                        <ext:TextField ID="txtTcity" runat="server" FieldLabel="<%$ Resources:LocalText,To%>"
                                            LabelWidth="70" Width="280" X="320" Y="40" MaxLength="50">
                                        </ext:TextField>
                                        <ext:Label ID="Label8" runat="server" Width="85" X="10" Y="73" Text="<%$ Resources:LocalText,File1%>"
                                            Cls="UploadLabel" />
                                        <ext:FileUploadField ID="FileUploadField1" runat="server" Icon="Attach" X="80" Y="60"
                                            Width="150" ButtonText="" />
                                        <ext:Button ID="btnUploadScanFile" runat="server" X="243" Y="70" Icon="Add">
                                            <DirectEvents>
                                        ...
                                        <ext:Button ID="Button1" runat="server" X="268" Y="70" Icon="Delete">
                                            <Listeners>
                                                <Click Handler="HLAttachFile.setText('');hdScanFile.setValue('');" />
                                            </Listeners>
                                        </ext:Button>
                                        <ext:HyperLink ID="HLAttachFile" runat="server" Text="" X="298" Y="73" Target="_blank" />
                                        <ext:TextField ID="txtCOAContent" runat="server" FieldLabel="<%$ Resources:LocalText,COARemark%>"
                                            X="10" Y="100" Anchor="100%" LabelWidth="80" MaxLength="100">
                                        </ext:TextField>
            ...
            <ext:GridPanel ID="GridPanel1" runat="server" StoreID="Store1" Title="<%$ Resources:LocalText,ExpenseList%>"
                TrackMouseOver="false" Height="150" X="10" Y="203" AutoScroll="true">
                <ColumnModel ID="ColumnModel1" runat="server">
                    <Columns>
                        <ext:CommandColumn Width="50" ButtonAlign="Center" Sortable="False">
                            <Commands>
                        ...
                        <ext:Column Header="<%$ Resources:LocalText,Amount%>" Width="100" DataIndex="Amount" Sortable="False">
                            <Renderer Fn="GetNumber" />
                        </ext:Column>
                        <%--<ext:Column Header="<%$ Resources:LocalText,CostCenter%>" Width="120" DataIndex="TSation" Sortable="False"/>--%>
                        <ext:Column Header="<%$ Resources:LocalText,COARemark%>" Width="100" DataIndex="AccountDes" Sortable="False"/>
                        <ext:DateColumn Header="<%$ Resources:LocalText,Date%>" Width="80" DataIndex="Tdate"
                            Format="yyyy/MM/dd" Sortable="False"/>
                        <ext:Column Header="<%$ Resources:LocalText,From%>" Width="200" DataIndex="Fcity" Sortable="False"/>
                        <ext:Column Header="<%$ Resources:LocalText,To%>" Width="200" DataIndex="Tcity" Sortable="False"/>
                        <ext:Column Header="<%$ Resources:LocalText,File%>" Width="160" DataIndex="Attach" Sortable="False">
                ...
                <Listeners>
                    ...
                    <Render Handler="if(parent.AddSubStore('T')){EditBudgetCurrentStatus();}" />
                </Listeners>
            </ext:GridPanel>
