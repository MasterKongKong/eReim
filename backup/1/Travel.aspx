...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        var SaveAll1 = function (a,warningmsg) {
            if (a == 'ND') {
                //正式申请
                ...
                RM.SaveAll1(a, detail, CCMailListdetail, header0string, header1string, header2string, LabelCurrency.getText(), labelDepartment.getText(),warningmsg, {
                    eventMask: {
                        showMask: true,
                        tartget: "Page"
                    }
        ...
        var SelectCell = function (a, row, column) {
            ...
            if (column % 2 == 1) {
                ...
                //                var costtype = a.selection.record.data.Category;
                //                var r = row.toString();
                if (GridView1.headerRows[2].columns[column + 1].component.isValid()) {
                    var date = GridView1.headerRows[2].columns[column + 1].component.getRawValue();
                    if (date != "") {
                        if (hdOwner.getValue().toString() == '') {
                            lbCOA.setText('Please select eLeave Data.');
                            return false;
                        }
                        var costcenter = GridView1.headerRows[1].columns[column].component.getValue();
                        var r = row.toString();
                        RM.LoadBudget(costcenter, r, date, costtype, {
                            timeout: 30000
                        });
                    }
                    else {
                        //                        Ext.Msg.show({ title: 'Message', msg: 'Please input valid Date.', buttons: { ok: 'Ok'} });
                        lbCOA.setText('Please input valid Date.');
                        return false;
                    }
                }
                else {
                ...
                //                var costtype = a.selection.record.data.Category;
                if (GridView1.headerRows[2].columns[column].component.isValid()) {
                    var date = GridView1.headerRows[2].columns[column].component.getRawValue();
                    if (date != "") {
                        if (hdOwner.getValue().toString() == '') {
                            lbCOA.setText('Please select eLeave Data.');
                            return false;
        ...
        var PasseLeaveData = function (command, record, rowIndex) {
            ...
            hdCurrency.setValue(record.data.CurrencyBudget); //记录成本中心汇率
            var headercol = GridPanel2.view.headerRows[0].columns.length;
            //出差站点不能为空
                ...
                <Items>
                    <ext:BorderLayout ID="BorderLayout1" runat="server">
                        <West Collapsible="true" Split="true" CollapseMode="Mini">
                            <ext:Panel ID="Panel2" runat="server" Width="195" Layout="FitLayout" Title="<a>Test Web Site</a>">
                                <Items>
                        ...
                        <Center>
                            <ext:Panel ID="Panel3" runat="server" Title="<%$ Resources:LocalText,TravellingExpenseApply%>"
                                ...
                                <Items>
                                    ...
                                    <ext:GridPanel ID="GridPanelBudget" runat="server" TrackMouseOver="false" Height="110" X="10" Y="550" AutoScroll="true">
                                        <Store>
